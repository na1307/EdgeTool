...
using Mygod.Windows.Automation.Peers;
    ...
    public partial class AutoCompleteBox : Control, IUpdateVisualState
    {
        ...
        private static void OnMinimumPopulateDelayPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (newValue < 0)
            {
                ...
                throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, "Invalid MinimumPopulateDelay value '{0}'. The value must be greater than or equal to zero.", newValue), "value");
            }
        ...
        private static void OnMaxDropDownHeightPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            ...
            if (newValue < 0)
            {
                ...
                throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, "Invalid maximum drop down height value '{0}'. The value must be greater than or equal to zero.", e.NewValue), "value");
            }
        ...
        private static void OnFilterModePropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
                ...
                mode != AutoCompleteFilterMode.StartsWithOrdinalCaseSensitive)
            {
                ...
                throw new ArgumentException("Invalid FilterMode enumeration value. The value must be one of the defined AutoCompleteFilterMode values to be accepted.", "value");
            }
        ...
        {
            point = new Point(point.X - Margin.Left - BorderThickness.Left - Padding.Left, point.Y - Margin.Top - BorderThickness.Top - Padding.Top);
            return TextBox.GetCharacterIndexFromPoint(point, snapToText);
        }
    }
}