...
namespace Mygod.Windows
{
    ...
    public static class DragSourceHelper
    {
        ...
        /// </summary>
        private static readonly IDictionary<UIElement, DragSourceEntry> s_dataContext = new Dictionary<UIElement, DragSourceEntry>();

        /// <summary>
        /// Keeps drop description info for a data object.
        /// </summary>
        private static readonly IDictionary<IDataObject, DropDescriptionFlags> s_dropDescriptions = new Dictionary<IDataObject, DropDescriptionFlags>();
        ...
        {
            return new System.Windows.DataObject(new Mygod.Windows.DataObject());
        }
        ...
        public static void RegisterDefaultDragSource(UIElement dragSource, IDataObject data)
        {
            ...
            entry.adviseConnection = ((System.Runtime.InteropServices.ComTypes.IDataObject) data).Advise(new AdviseSink(data), DropDescriptionFormat, 0);

            // Hook up the default drag source event handlers
            dragSource.GiveFeedback += DefaultGiveFeedbackHandler;
            dragSource.QueryContinueDrag += DefaultQueryContinueDragHandler;
        }
        ...
        {
            return DoDragDropInternal(dragSource, RegisterDefaultDragSource(dragSource, cursorOffset), allowedEffects, data);
            //return DoDragDrop(dragSource, Capture(dragSource), cursorOffset, allowedEffects, data);
        }
        ...
        /// <param name="data">The associated data.</param>
        /// <returns>The accepted drop effects from the completed operation.</returns>
        private static DragDropEffects DoDragDropInternal(UIElement dragSource, IDataObject dataObject, DragDropEffects allowedEffects, IEnumerable<KeyValuePair<string, object>> data)
        {
