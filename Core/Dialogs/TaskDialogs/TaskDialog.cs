...
namespace Microsoft.WindowsAPICodePack.Dialogs
{
    ...
    public sealed class TaskDialog : IDialogControlHost, IDisposable
    {
        ...
        public static TaskDialogResult Show(Window owner, string title, string mainInstruction, string content = null, TaskDialogType type = TaskDialogType.Custom, string expandedInfo = null, string verificationText = null, string footerText = null, TaskDialogStandardButtons buttons = TaskDialogStandardButtons.None, int? defaultButtonId = null, TaskDialogStandardIcon mainIcon = TaskDialogStandardIcon.None, TaskDialogStandardIcon footerIcon = TaskDialogStandardIcon.None)
        {
            if (TaskDialog.staticDialog == null)
            {
                TaskDialog.staticDialog = new TaskDialog();
            }
            TaskDialog.staticDialog.OwnerWindowHandle = ((owner == null) ? IntPtr.Zero : new WindowInteropHelper(owner).Handle);
            TaskDialog.staticDialog.Caption = title;
            TaskDialog.staticDialog.InstructionText = mainInstruction;
            TaskDialog.staticDialog.Text = content ?? string.Empty;
            TaskDialog.staticDialog.DetailsExpandedText = expandedInfo;
            TaskDialog.staticDialog.FooterCheckBoxText = verificationText;
            TaskDialog.staticDialog.FooterText = footerText;
            TaskDialog.staticDialog.StandardButtons = ((type == TaskDialogType.Custom || buttons != TaskDialogStandardButtons.None) ? buttons : TaskDialog.GetButtons(type));
            if (defaultButtonId != null)
            {
                TaskDialog.staticDialog.DefaultButtonId = defaultButtonId.Value;
            }
            TaskDialog.staticDialog.Icon = ((type == TaskDialogType.Custom || mainIcon != TaskDialogStandardIcon.None) ? mainIcon : TaskDialog.GetIcon(type));
            TaskDialog.staticDialog.FooterIcon = ((type == TaskDialogType.Custom || footerIcon != TaskDialogStandardIcon.None) ? footerIcon : TaskDialog.GetIcon(type));
            return TaskDialog.staticDialog.Show();
        }

        private static TaskDialogStandardIcon GetIcon(TaskDialogType type)
        {
            switch (type)
            {
                case TaskDialogType.Information:
                case TaskDialogType.YesNoQuestion:
                case TaskDialogType.YesNoCancelQuestion:
                case TaskDialogType.OKCancelQuestion:
        ...
        private static TaskDialogStandardButtons GetButtons(TaskDialogType type)
        {
            switch (type)
            {
                case TaskDialogType.YesNoQuestion:
        ...
        public int DefaultButtonId { get; set; }
